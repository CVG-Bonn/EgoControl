...
class VideoCondition(TextCondition):
    ...
    def edit_for_inference(self, is_cfg_conditional: bool = True, num_conditional_frames: int = 1) -> VideoCondition:
        ...
        # bring back guidance for conditional frames
...
@dataclass(frozen=True)
class PoseCondition(VideoCondition):
...
class PoseConditioner(VideoConditioner):
    def forward(
        self,
        batch: dict,
        override_dropout_rate: dict[str, float] | None = None,
        ...
        output = super()._forward(batch, override_dropout_rate)
        assert "pose" in batch, "PoseConditioner requires 'pose' in batch"
        output["pose"] = batch["pose"]
        return PoseCondition(**output)
