...
class Predict2Video2WorldPoseConditionedModel(Predict2Video2WorldModel):
    def __init__(self, config: Predict2Video2WorldModelConfig):
        ...
        # NOTE: replace the pipeline with Pose-conditioned setup
        self.pipe = Video2WorldPoseConditionedPipeline.from_config(
            config.pipe_config,
        ...
        if config.train_architecture == "lora":
            log.critical("Training with LoRA")
        ...
        log.info('Pose conditioned model')
        log.info(
