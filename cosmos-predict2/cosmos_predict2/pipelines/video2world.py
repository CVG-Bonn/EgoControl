...
from imaginaire.auxiliary.text_encoder import (
    CosmosTextEncoder,
    get_cosmos_text_encoder, CosmosTextEncoderConfig,
)
...
class Video2WorldPipeline(BasePipeline):
    ...
    def _get_data_batch_input(
        ...
        data_batch = {
            ...
            "t5_text_embeddings": torch.zeros(self.batch_size,
                                              CosmosTextEncoderConfig.NUM_TOKENS,
                                              CosmosTextEncoderConfig.EMBED_DIM,
                                              dtype=torch.bfloat16) if prompt == "" else self.encode_prompt(prompt).to(dtype=self.torch_dtype),
    ...
    def _normalize_video_databatch_inplace(self, data_batch: dict[str, torch.Tensor], input_key: str = None) -> None:  # noqa: RUF013
        ...
        if data_batch[input_key].shape[1] != 3:
            return # Skip if not RGB video, e.g. latent already
